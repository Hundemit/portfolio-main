      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 dark:bg-white/20 bg-black/20 z-50"
          ...
          <motion.div
            ...
            className={cn("fixed bottom-3 left-0 right-0 bg-background border-t border-border rounded-lg  max-h-[70vh] overflow-hidden w-[90%] mx-auto flex flex-col z-[51]", className)}>
            {children}
          </motion.div>
        </>
      )}
...
export function DrawerHeader({ children, className, showCloseButton = true }: DrawerHeaderProps) {
  ...
  return (
    <div className={cn("flex items-center justify-between p-4 border-b border-border", className)}>
      ...
      {showCloseButton && (
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsOpen(false)} className="text-muted-foreground hover:text-foreground transition-colors ml-4">
          <X size={20} />
        </motion.button>
      )}
    </div>
